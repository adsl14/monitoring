...
def etiquetar(path_radar,epoch,areas,output_writer,num_areas,actual):
  ...
  for area in areas:
    ...
    for index in indexes_sentinel2:
      ...
      plt.plot(dates_s2,data_s2,label=index)
    ...
    for i in range(0,num_files_radar):
      ...
      plt.figure(figsize=(10,5))
      plt.plot(dates_s1[i],data_s1,label=indexes_sentinel1_v2[i])
      plt.xticks(rotation=25)
      plt.grid()
      plt.legend()
...
for campaing in campaings:
  ...
  if not os.path.exists(tags_path):
    with open(tags_path, mode='w') as output_file:
  ...
  # if 'tags.csv' exists
    ...
    if num_areas > 0:
      ...
      with open(tags_path, mode='r') as file_input:
        with open(tags_path_temp, mode='w') as output_file:
          output_writer = csv.writer(output_file,delimiter=',')
          output_writer.writerow(['id',labels_header])
