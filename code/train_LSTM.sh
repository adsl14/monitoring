			...
			do
				...
				do
					...
					do
						...
						do
							python train.py --network="LSTM" --percentageGPU=$percentageGPU --learning_rate=${lrs[lr]} --batch_size=${batch_sizes[bs]} --epochs=${epochs[e]} --percentageDropout=${dropouts[dr]} --patience=${patiences[p]} --patience_reduce_lr=${patiences_reduce_lr[pr_lr]} --nNeurons=${nNeurons[n]} &
							wait
						done
